    ...
    <header className={cn(WEBSITE_BOX, 'mt-20 xl:mt-16 sm:mt-3', 'flex sm:flex-col items-center sm:items-start justify-between sm:gap-4.5')}>
      ...
      <div className="flex gap-1.25 sm:w-full">
        {Object.entries(navigation).map(([key, item]) => {
          const isTextButton = typeof item.content === 'string'
          return (
            <Button
              as="link"
              href={item.link}
              variant={!isTextButton ? 'primary' : 'light'}
            ...
            >
              {!isTextButton ? (
                <Image className="size-5.5 sm:min-w-5.5 aspect-square group-hover:scale-[1.1] duration-300" src={item.content} alt={`bozzhik ${key} icon`} />
              ) : (
                <SPAN className="px-1.5 sm:px-2.25 sm:py-[1.5px] text-foreground font-semibold group-hover:scale-[1.04] duration-300">{item.content}</SPAN> // string
              )}
            </Button>
          )
        })}
